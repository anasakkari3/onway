...
export default function NotificationListClient({ initialNotifications }: { initialNotifications: NotificationsRow[] }) {
    ...
    const handleMarkAsRead = async (id: string, e: React.MouseEvent) => {
        ...
        try {
            ...
            // Restore state if failed (optimistic UI), but non-critical
        }
    };
                ...
                {notifications.map(n => {
                    ...
                    const CardContent = (
                        <div className={`p-4 rounded-2xl border ${n.is_read ? 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800' : 'bg-white dark:bg-slate-800 border-sky-200 dark:border-sky-800 shadow-sm'} flex items-start gap-4 transition-colors`}>
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className={`text-sm font-bold truncate ${n.is_read ? 'text-slate-700 dark:text-slate-300' : 'text-slate-900 dark:text-white'}`}>{n.title}</h3>
                                <p className={`text-sm mt-1 leading-relaxed ${n.is_read ? 'text-slate-500 dark:text-slate-400' : 'text-slate-600 dark:text-slate-300'}`}>{n.body}</p>
                                <p className="text-[10px] uppercase font-semibold text-slate-400 dark:text-slate-500 mt-2 tracking-wider">
                                    {new Date(n.created_at).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </p>
                            </div>
                            {!n.is_read && (
                                <button onClick={(e) => handleMarkAsRead(n.id, e)} className="shrink-0 p-2 rounded-full hover:bg-sky-50 dark:hover:bg-sky-900/30 text-sky-500 transition-colors" title="Mark as read">
                    ...
                    return n.link_url ? (
                        <Link 
                            ...
                            onClick={() => { if (!n.is_read) { setNotifications(prev => prev.map(item => item.id === n.id ? { ...item, is_read: true } : item)); markAsRead(n.id); } }}
                        >
                            {CardContent}
                        </Link>
                    ) : (
